...
public class PlantWateringService extends IntentService {
    ...
    //  (1): Change ACTION_WATER_PLANTS to ACTION_WATER_PLANT and
     ...
     * @see IntentService
     */
    public static void startActionWaterPlants(Context context, int plantId ) {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (ACTION_WATER_PLANT.equals(action)) {
                handleActionWaterPlants(intent.getLongExtra(EXTRA_PLANT_ID, INVALID_PLANT_ID));
    ...
    private void handleActionWaterPlants(long plantId) {
        Uri PLANT_URI = ContentUris.appendId(BASE_CONTENT_URI.buildUpon().appendPath(PATH_PLANTS), plantId).build();
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            showWater = timeNow - wateredAt > MIN_AGE_BETWEEN_WATER;
            cursor.close();
        ...
        PlantWidgetProvider.updatePlantWidgets(this, appWidgetManager, imgRes, appWidgetIds, plantId, showWater);
    }
}
